(function(){
  ...
  document.querySelectorAll('.skillitem').forEach(function(el){
    skillIO.observe(el);
  });
  ...
  document.querySelectorAll('.filterbtn').forEach(function(btn){
    btn.addEventListener('click', function(){
      ...
      // re-check if grid needs column adjustment after filter
      const visible = document.querySelectorAll('#projgrid .card[style*="flex"]');
      const grid = document.getElementById('projgrid');
      if(visible.length === 1){
        grid.style.gridTemplateColumns = 'repeat(1, minmax(0, 480px))';
  ...
  document.getElementById('contactform').addEventListener('submit', async function(e){
    ...
    // loading state
    btn.innerHTML = '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Sending...';
    btn.disabled = true;
  ...
  if(window.matchMedia('(hover: hover) and (pointer: fine)').matches){
    ...
    window.addEventListener('mousemove', function(e){
      ...
      cursor.style.background = isInteractive
        ? 'rgba(201,151,63,.15)'
        : 'rgba(201,151,63,.55)';
      cursor.style.transform = 'translate(' + (mx - size / 2) + 'px,' + (my - size / 2) + 'px)';
    });
